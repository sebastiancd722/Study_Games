...
import { Flashcard, GameStatus, GridCell, GameState, GameMode } from '../../types';
import { validateAnswer } from '../../services/geminiService';
import { Button } from '../Button';
import { Trophy, Flame, CheckCircle, XCircle, HelpCircle, AlertTriangle } from 'lucide-react';
...
export const GridGame: React.FC<GridGameProps> = ({ cards, onExit, initialGrid, initialGameState }) => {
  ...
  // Initialization Logic (Load from props or generate new)
  useEffect(() => {
  ...
  }, [cards, initialGrid, initialGameState]);
  ...
  useEffect(() => {
    if (gameState.cellsCleared === gameState.totalCells && gameState.totalCells > 0) {
      ...
      localStorage.removeItem('studygrid_save'); // Clear save on win
    }
  ...
  const closeAndApplyResult = () => {
    ...
    // Wait for animation to finish before applying state changes
    setTimeout(() => {
        if (gradingResult.score >= 50) {
          // CLEAR LOGIC (Pass or Partial)
        ...
        setGradingResult(null);
        setIsClosing(false);
    }, 250); // Matches .animate-modal-exit duration
  };
        ...
        {grid.map((cell, index) => (
          <button
            ...
            style={{ 
              ...
              // Ensure cleared cells keep their state appearance without re-animating on mount if restoring from save
              animationFillMode: 'forwards'
            }}
            className={`
              aspect-square rounded-2xl relative group outline-none
              ${cell.isCleared 
                ? 'animate-cell-clear pointer-events-none' // Use animation for cleared state
