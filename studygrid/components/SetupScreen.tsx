...
import { Flashcard, GameMode, GridCell, GameState, SavedGameData } from '../types';
import { Button } from './Button';
import { Sparkles, Play, FileText, AlertCircle, UploadCloud, X, File as FileIcon, Image as ImageIcon, Presentation, Grid, Brain, Zap, RotateCcw, Info, BookOpen } from 'lucide-react';
import JSZip from 'jszip';

interface SetupScreenProps {
  onStartGame: (cards: Flashcard[], mode: GameMode, savedGrid?: GridCell[], savedGameState?: GameState) => void;
}

export const SetupScreen: React.FC<SetupScreenProps> = ({ onStartGame }) => {
  const [mode, setMode] = useState<'ai' | 'manual'>('ai');
  const [gameMode, setGameMode] = useState<GameMode>(GameMode.LEARN); // Default to Learn
  const [topic, setTopic] = useState('');
  const [manualText, setManualText] = useState('');
  const [aiContextText, setAiContextText] = useState(''); // New state for AI text input
  ...
  const handleResume = () => {
    if (savedGame) {
      onStartGame(savedGame.cards, savedGame.mode, savedGame.grid, savedGame.gameState);
    }
  };
  ...
  const handleStart = async () => {
    ...
    setLoading(false);
    // Clear old save if starting new game
    localStorage.removeItem('studygrid_save');
    onStartGame(cards, gameMode);
  };
    ...
    <div className="max-w-4xl mx-auto w-full p-4 md:p-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
            ...
            {/* GAME SECTION */}
            <div className="space-y-3">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider pl-1 mt-4">2. Play & Test</p>
            ...
            {error && (
              <div className="bg-rose-950/30 border border-rose-500/30 text-rose-300 p-4 rounded-xl flex items-center gap-3 animate-fade-in text-sm">
                <AlertCircle className="w-5 h-5 shrink-0" />
                <span>{error}</span>
              </div>
            )}

            <Button onClick={handleStart} isLoading={loading} className="w-full py-4 text-lg shadow-xl shadow-indigo-900/20">
              <Play className="w-5 h-5" />
              {gameMode === GameMode.LEARN ? 'Start Studying' : 'Start Game'}
            </Button>
            <p className="text-center text-xs text-slate-500">Powered by Google Gemini</p>
        </div>
      </div>
    </div>
  );
